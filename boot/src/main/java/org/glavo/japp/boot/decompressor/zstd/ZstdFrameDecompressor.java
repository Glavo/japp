...
import static org.glavo.japp.boot.decompressor.zstd.Constants.COMPRESSED_BLOCK;
import static org.glavo.japp.boot.decompressor.zstd.Constants.COMPRESSED_LITERALS_BLOCK;
import static org.glavo.japp.boot.decompressor.zstd.Constants.DEFAULT_MAX_OFFSET_CODE_SYMBOL;
import static org.glavo.japp.boot.decompressor.zstd.Constants.LITERALS_LENGTH_BITS;
import static org.glavo.japp.boot.decompressor.zstd.Constants.LITERAL_LENGTH_TABLE_LOG;
import static org.glavo.japp.boot.decompressor.zstd.Constants.LONG_NUMBER_OF_SEQUENCES;
import static org.glavo.japp.boot.decompressor.zstd.Constants.MAGIC_NUMBER;
import static org.glavo.japp.boot.decompressor.zstd.Constants.MATCH_LENGTH_BITS;
import static org.glavo.japp.boot.decompressor.zstd.Constants.MATCH_LENGTH_TABLE_LOG;
import static org.glavo.japp.boot.decompressor.zstd.Constants.MAX_BLOCK_SIZE;
import static org.glavo.japp.boot.decompressor.zstd.Constants.MAX_LITERALS_LENGTH_SYMBOL;
import static org.glavo.japp.boot.decompressor.zstd.Constants.MAX_MATCH_LENGTH_SYMBOL;
import static org.glavo.japp.boot.decompressor.zstd.Constants.MIN_BLOCK_SIZE;
import static org.glavo.japp.boot.decompressor.zstd.Constants.MIN_SEQUENCES_SIZE;
import static org.glavo.japp.boot.decompressor.zstd.Constants.MIN_WINDOW_LOG;
import static org.glavo.japp.boot.decompressor.zstd.Constants.OFFSET_TABLE_LOG;
import static org.glavo.japp.boot.decompressor.zstd.Constants.RAW_BLOCK;
import static org.glavo.japp.boot.decompressor.zstd.Constants.RAW_LITERALS_BLOCK;
import static org.glavo.japp.boot.decompressor.zstd.Constants.RLE_BLOCK;
import static org.glavo.japp.boot.decompressor.zstd.Constants.RLE_LITERALS_BLOCK;
import static org.glavo.japp.boot.decompressor.zstd.Constants.SEQUENCE_ENCODING_BASIC;
import static org.glavo.japp.boot.decompressor.zstd.Constants.SEQUENCE_ENCODING_COMPRESSED;
import static org.glavo.japp.boot.decompressor.zstd.Constants.SEQUENCE_ENCODING_REPEAT;
import static org.glavo.japp.boot.decompressor.zstd.Constants.SEQUENCE_ENCODING_RLE;
import static org.glavo.japp.boot.decompressor.zstd.Constants.SIZE_OF_BLOCK_HEADER;
import static org.glavo.japp.boot.decompressor.zstd.Constants.SIZE_OF_BYTE;
import static org.glavo.japp.boot.decompressor.zstd.Constants.SIZE_OF_INT;
...
import static org.glavo.japp.boot.decompressor.zstd.Constants.TREELESS_LITERALS_BLOCK;

import static org.glavo.japp.boot.decompressor.zstd.Util.fail;
...
public final class ZstdFrameDecompressor {
    ...
    public int decompress(
        ...
        while (input < inputLimit) {
            ...
            if (frameHeader.hasChecksum) {
                ...
                int checksum = MemoryAccess.getInt(inputBase, input);
    ...
    static int decodeRawBlock(Object inputBase, long inputAddress, int blockSize, Object outputBase, long outputAddress, long outputLimit) {
        ...
        MemoryAccess.copyMemory(inputBase, inputAddress, outputBase, outputAddress, blockSize);
        return blockSize;
    }
    ...
    static int decodeRleBlock(int size, Object inputBase, long inputAddress, Object outputBase, long outputAddress, long outputLimit) {
        ...
        long output = outputAddress;
        long value = MemoryAccess.getByte(inputBase, inputAddress) & 0xFFL;
        ...
        if (remaining >= SIZE_OF_LONG) {
            ...
            do {
                MemoryAccess.putLong(outputBase, output, packed);
                output += SIZE_OF_LONG;
        ...
        for (int i = 0; i < remaining; i++) {
            MemoryAccess.putByte(outputBase, output, (byte) value);
            output++;
        }
    ...
    int decodeCompressedBlock(
        ...
        // decode literals
        int literalsBlockType = MemoryAccess.getByte(inputBase, input) & 0b11;
    ...
    private int decompressSequences(
        ...
        // decode header
        int sequenceCount = MemoryAccess.getByte(inputBase, input++) & 0xFF;
        if (sequenceCount != 0) {
            if (sequenceCount == 255) {
                verify(input + SIZE_OF_SHORT <= inputLimit, input, "Not enough input bytes");
                sequenceCount = (MemoryAccess.getShort(inputBase, input) & 0xFFFF) + LONG_NUMBER_OF_SEQUENCES;
                input += SIZE_OF_SHORT;
            } else if (sequenceCount > 127) {
                verify(input < inputLimit, input, "Not enough input bytes");
                sequenceCount = ((sequenceCount - 128) << 8) + (MemoryAccess.getByte(inputBase, input++) & 0xFF);
            }

            verify(input + SIZE_OF_INT <= inputLimit, input, "Not enough input bytes");

            byte type = MemoryAccess.getByte(inputBase, input++);
    ...
    private static long copyLastLiteral(Object outputBase, Object literalsBase, long literalsLimit, long output, long literalsInput) {
        ...
        MemoryAccess.copyMemory(literalsBase, literalsInput, outputBase, output, lastLiteralsSize);
    ...
    private static void copyMatchTail(Object outputBase, long fastOutputLimit, long output, long matchOutputLimit, long matchAddress, int matchLength, long fastMatchOutputLimit) {
        ...
        if (matchOutputLimit < fastMatchOutputLimit) {
            int copied = 0;
            do {
                MemoryAccess.putLong(outputBase, output, MemoryAccess.getLong(outputBase, matchAddress));
                output += SIZE_OF_LONG;
            ...
            while (output < matchOutputLimit) {
                MemoryAccess.putByte(outputBase, output++, MemoryAccess.getByte(outputBase, matchAddress++));
            }
        }
    }
    ...
    private static long copyMatchHead(Object outputBase, long output, int offset, long matchAddress) {
        ...
        if (offset < 8) {
            ...
            MemoryAccess.putByte(outputBase, output, MemoryAccess.getByte(outputBase, matchAddress));
            MemoryAccess.putByte(outputBase, output + 1, MemoryAccess.getByte(outputBase, matchAddress + 1));
            MemoryAccess.putByte(outputBase, output + 2, MemoryAccess.getByte(outputBase, matchAddress + 2));
            MemoryAccess.putByte(outputBase, output + 3, MemoryAccess.getByte(outputBase, matchAddress + 3));
            matchAddress += increment32;

            MemoryAccess.putInt(outputBase, output + 4, MemoryAccess.getInt(outputBase, matchAddress));
    ...
    private static long copyLiterals(Object outputBase, Object literalsBase, long output, long literalsInput, long literalOutputLimit) {
        ...
        do {
            MemoryAccess.putLong(outputBase, output, MemoryAccess.getLong(literalsBase, literalInput));
            output += SIZE_OF_LONG;
    ...
    private long computeMatchLengthTable(int matchLengthType, Object inputBase, long input, long inputLimit) {
        switch (matchLengthType) {
            case SEQUENCE_ENCODING_RLE:
                ...
                byte value = MemoryAccess.getByte(inputBase, input++);
    ...
    private void executeLastSequence(Object outputBase, long output, long literalOutputLimit, long matchOutputLimit, long fastOutputLimit, long literalInput, long matchAddress) {
        ...
        while (output < literalOutputLimit) {
            MemoryAccess.putByte(outputBase, output, MemoryAccess.getByte(literalsBase, literalInput));
            output++;
            literalInput++;
        }
    ...
    private int decodeCompressedLiterals(Object inputBase, final long inputAddress, int blockSize, int literalsBlockType) {
        ...
        int headerSize;
        int type = (MemoryAccess.getByte(inputBase, input) >> 2) & 0b11;
        switch (type) {
            case 0:
                singleStream = true;
            case 1: {
                int header = MemoryAccess.getInt(inputBase, input);
            ...
            case 3: {
                ...
                long header = MemoryAccess.getByte(inputBase, input) & 0xFF |
                              (MemoryAccess.getInt(inputBase, input + 1) & 0xFFFF_FFFFL) << 8;
    ...
    private int decodeRleLiterals(Object inputBase, final long inputAddress, int blockSize) {
        ...
        switch (type) {
            case 0:
            case 2:
                outputSize = (MemoryAccess.getByte(inputBase, input) & 0xFF) >>> 3;
                input++;
                break;
            case 1:
                outputSize = (MemoryAccess.getShort(inputBase, input) & 0xFFFF) >>> 4;
                input += 2;
                break;
            case 3:
                // we need at least 4 bytes (3 for the header, 1 for the payload)
                verify(blockSize >= SIZE_OF_INT, input, "Not enough input bytes");
                outputSize = (MemoryAccess.getInt(inputBase, input) & 0xFF_FFFF) >>> 4;
                input += 3;
                break;
            default:
    ...
    private int decodeRawLiterals(Object inputBase, final long inputAddress, long inputLimit) {
        ...
        switch (type) {
            case 0:
            case 2:
                literalSize = (MemoryAccess.getByte(inputBase, input) & 0xFF) >>> 3;
                input++;
                break;
            case 1:
                literalSize = (MemoryAccess.getShort(inputBase, input) & 0xFFFF) >>> 4;
                input += 2;
                break;
            case 3:
                // read 3 little-endian bytes
                int header = ((MemoryAccess.getByte(inputBase, input) & 0xFF) |
                              ((MemoryAccess.getShort(inputBase, input + 1) & 0xFFFF) << 8));
        ...
        if (literalSize > (inputLimit - input) - SIZE_OF_LONG) {
            ...
            MemoryAccess.copyMemory(inputBase, input, literals, literalsAddress, literalSize);
    ...
    static FrameHeader readFrameHeader(final Object inputBase, final long inputAddress, final long inputLimit) {
        ...
        int frameHeaderDescriptor = MemoryAccess.getByte(inputBase, input++) & 0xFF;
        ...
        int windowSize = -1;
        if (!singleSegment) {
            int windowDescriptor = MemoryAccess.getByte(inputBase, input++) & 0xFF;
        ...
        switch (dictionaryDescriptor) {
            case 1:
                dictionaryId = MemoryAccess.getByte(inputBase, input) & 0xFF;
                input += SIZE_OF_BYTE;
                break;
            case 2:
                dictionaryId = MemoryAccess.getShort(inputBase, input) & 0xFFFF;
                input += SIZE_OF_SHORT;
                break;
            case 3:
                dictionaryId = MemoryAccess.getInt(inputBase, input) & 0xFFFF_FFFFL;
                input += SIZE_OF_INT;
                break;
        }
        ...
        switch (contentSizeDescriptor) {
            case 0:
                if (singleSegment) {
                    contentSize = MemoryAccess.getByte(inputBase, input) & 0xFF;
                    input += SIZE_OF_BYTE;
                }
                break;
            case 1:
                contentSize = MemoryAccess.getShort(inputBase, input) & 0xFFFF;
                contentSize += 256;
                input += SIZE_OF_SHORT;
                break;
            case 2:
                contentSize = MemoryAccess.getInt(inputBase, input) & 0xFFFF_FFFFL;
                input += SIZE_OF_INT;
                break;
            case 3:
                contentSize = MemoryAccess.getLong(inputBase, input);
                input += SIZE_OF_LONG;
                break;
        }
    ...
    static int verifyMagic(Object inputBase, long inputAddress, long inputLimit) {
        ...
        int magic = MemoryAccess.getInt(inputBase, inputAddress);
    ...
    public int decompress(ByteBuffer input, ByteBuffer output) throws MalformedInputException {
        Object inputBase;
        ...
        if (input.hasArray()) {
            ...
            inputBase = null;
            inputBaseAddress = MemoryAccess.getDirectBufferAddress(input);
        }
        ...
        if (output.hasArray()) {
            ...
            outputBase = null;
            outputBaseAddress = MemoryAccess.getDirectBufferAddress(output);
        }
        outputAddress = outputBaseAddress + output.position();
        outputLimit = outputBaseAddress + output.limit();

        input.position(input.limit());
        int n = decompress(inputBase, inputAddress, inputLimit, outputBase, outputAddress, outputLimit);
